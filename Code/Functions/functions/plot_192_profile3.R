# 2020-03-31: Implemented change to accept single sample 
plot_192_profile3 = function(mut_matrix,colors,ymax = 0.1,condensed = TRUE)
{
  ...
  if (grepl("transcribed", df3$strand[1])) {
  df3$strand = factor(df3$strand, levels = c("transcribed", "untranscribed"))
  } 
  
  if (grepl("left|right", df3$strand[1])){
    df3$strand = factor(df3$strand, levels = c("left", "right"))
  } 
