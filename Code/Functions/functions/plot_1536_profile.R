...
plot_1536_profile <- function (mut_matrix, colors, ymax = 0.02, condensed = TRUE) {
  ...
  if (!dim(mut_matrix)[1] == 1536) {
    stop("Provide 1536 mutation matrix")
  }
  context = CONTEXTS_1536
  substitution = rep(SUBSTITUTIONS, each = 256)
  ...
  value = NULL
  if (condensed) {
    plot = ggplot(data = df3, aes(x = context, y = value, fill = substitution, width = 1)) + geom_bar(stat = "identity", 
